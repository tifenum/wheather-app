...
const fillimage = (refObj, obj, color) => {
    let { context, path } = refObj;
    context.beginPath();
    path(obj);
    const img = new Image(); // Create a new image object
    img.src = "https://unpkg.com/three-globe@2.35.3/example/img/earth-blue-marble.jpg";
    ...
    // Optional: Handle the case where the image fails to load
    img.onerror = () => {
        console.error("Image failed to load");
        context.fillStyle = color; // Fallback to a solid color
        context.fill();
    };
};
